...
WORDLIST_FILENAME = "/Users/karenyang/Documents/6.00.1x/ProblemSet4/words.txt"
...
def updateHand(hand, word):
    ...
    otherhand = hand.copy()
    for letter in word:
        newval = otherhand[letter]-1
        otherhand[letter]=newval
...
def isValidWord(word, hand, wordList):
    ...
    if word =="" or (not (word in wordList)):
        ...
        count = 0  
        for letter in word: 
            if not(letter in extrahand):
                return False
            else:
                if extrahand[letter]== 0:
                    return False
                else:
                    newval = extrahand[letter]-1
                    extrahand[letter]=newval
                    count +=1
        if count ==len(word):
            return True
        else:
...
def playHand(hand, wordList, n):
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    while calculateHandlen(hand) > 0:  # As long as there are still letters left in the hand:
        ...
        if word== ".":     # If the input is a single period:
            ...
            if isValidWord(word, hand, wordList)!=True:  # If the word is not valid:
                ...
                totalscore += a # Tell the user how many points the word earned, and the updated total score, in one line followed by a blank line
                print ("\""+ word + "\"" + " earned " + str(a) + " points. Total: " + str(totalscore)+ " points\n")
    ...
    if calculateHandlen(hand)==0:            
        ...
        if word!=".":
            a = getWordScore(word,n)          
            totalscore += a # Game is over (user entered a '.' or ran out of letters), so tell user the total score
...
Current Hand:  a a j l t w x

Enter word, or a '.' to indicate that you are finished: tax
"tax" earned 30 points. Total: 30 points

Current Hand:  a w j l

Enter word, or a '.' to indicate that you are finished: jaw
"jaw" earned 39 points. Total: 69 points
...
Goodbye! Total score: 69 points.

Enter n to deal a new hand, r to replay the last hand, or e to end game: r
Current Hand:  a a j l t w x

Enter word, or a '.' to indicate that you are finished: lax
"lax" earned 30 points. Total: 30 points

Current Hand:  a t w j

Enter word, or a '.' to indicate that you are finished: .
Goodbye! Total score: 30 points.

Enter n to deal a new hand, r to replay the last hand, or e to end game: n
Current Hand:  j l o u y x x

Enter word, or a '.' to indicate that you are finished: you
"you" earned 18 points. Total: 18 points

Current Hand:  x x j l
